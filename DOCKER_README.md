...
This guide explains how to build and run the digitalmatrix Digital Innovations application using Docker.
...
```bash
docker build -t digitalmatrix:latest .
```

Or with a specific version tag:

```bash
docker build -t digitalmatrix:1.0.0 .
```
...
```bash
docker run -d \
  --name digitalmatrix_app \
  -p 4019:4019 \
  --env-file .env.local \
  digitalmatrix:latest
```

Replace `digitalmatrix:latest` with your image name/tag.
...
| `TABLE_PREFIX`         | Database table prefix                | No       | `digitalmatrix_`            |
...
docker exec -it digitalmatrix_postgres psql -U postgres -d store_db
...
2. Or with `docker run`:
   ```bash
   docker run -p 8000:4019 digitalmatrix:latest
   ```
...
```bash
docker exec digitalmatrix_postgres psql -U postgres -d store_db -c "\dt"
```
...
```bash
docker rmi digitalmatrix:latest
```
