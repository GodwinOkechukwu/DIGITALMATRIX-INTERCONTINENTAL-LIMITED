...
This guide explains how to set up the GitHub Actions workflows for automated deployment of the digitalmatrix Digital Innovations application.
...
```bash
# Staging
docker logs -f digitalmatrix-staging

# Production
docker logs -f digitalmatrix-production
```
...
- Docker image tag: `slemfowel1/digitalmatrix-digital-innovations:staging`
- Container name: `digitalmatrix-staging`
- Port: `4019`
...
- Container name: `digitalmatrix-production`
- Port: `4019`
...
```bash
docker logs digitalmatrix-staging
```
...
docker exec digitalmatrix-production npm run migrate
```
...
- name: Run Database Migrations
  run: docker exec digitalmatrix-production npm run db:migrate
```
...
```bash
# On server
docker stop digitalmatrix-production
docker rm digitalmatrix-production
docker pull slemfowel1/digitalmatrix-digital-innovations:previous_tag
docker run -d -p 4019:4019 --name digitalmatrix-production \
  --env-file /home/apps/.env.production \
  slemfowel1/digitalmatrix-digital-innovations:previous_tag
```
